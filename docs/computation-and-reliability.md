...
相同幂等键的并发请求只能创建一个 `AnalysisRun`；其余请求返回相同 `run_id`。失败重试使用相同键，除非输入或版本发生实质变化。请求ID仅用于追踪，不参与计算身份。
...
以下变化自动产生新缓存键，不需要全局清空：事实版本、政策事务时间、假设、目标、分析期间、舍入方式、引擎、schema、模板或渲染器。涉及撤回授权或安全事件时，按内容索引删除所有关联缓存和派生产物。
