...
本文记录当日来源调研和规则包变更。后续实现状态以 [CHANGELOG](../CHANGELOG.md) 和下列 GitHub Issues 为准。
...
1. **广深失业保险费率**：2026 年度基准费率仍缺可核验原文，规则尚未建立；后续工作见 [Issue #2](https://github.com/Alaric1098/China-Pension-Strategy-Skill/issues/2)。
2. **医保基数口径差异**：当前已对养老和医疗基数越界发出非阻断警告，但金额仍按申报基数计算；真正按险种钳制见 [Issue #4](https://github.com/Alaric1098/China-Pension-Strategy-Skill/issues/4)。
3. **历史期费率**：广州和深圳 2024-2025 医疗费率已由独立历史包建模，并以 `historical: true` 与现行到期门禁区分。
4. **穗医保规字〔2022〕1号 到期监控**：广州医疗规则已标注 `effective_to: 2026-12-31`，维护脚本和分析提示均已覆盖规则级到期日。
