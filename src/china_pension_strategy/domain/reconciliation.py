...
class AggregatedCount:
    ...
    def __post_init__(self) -> None:
        _require_text(self.scheme, "scheme")
        _require_text(self.source_id, "source_id")
        if isinstance(self.reported_months, bool) or not isinstance(
            self.reported_months, int
        ):
            raise DomainValidationError(
                "reported_months must be a non-negative integer"
            )
...
class RecordConflict:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "assertion_refs", _tuple(self.assertion_refs, "assertion_refs")
        )
        object.__setattr__(
            self,
...
class ReconcileResult:
    ...
    def __post_init__(self) -> None:
        _require_text(self.scheme, "scheme")
        if isinstance(self.confirmed_months, bool) or not isinstance(
            self.confirmed_months, int
        ):
        ...
        object.__setattr__(
            self, "duplicates", _tuple(self.duplicates, "duplicates")
        )
        object.__setattr__(
            self, "conflicts", _tuple(self.conflicts, "conflicts")
        )
        object.__setattr__(
            self,
        ...
        if not all(isinstance(entry, ContributionMonth) for entry in self.duplicates):
            raise DomainValidationError(
                "duplicates must contain ContributionMonth values"
            )
        if not all(isinstance(conflict, RecordConflict) for conflict in self.conflicts):
            raise DomainValidationError(
                "conflicts must contain RecordConflict values"
            )
        if not all(
            isinstance(entry, ContributionMonth)
            for entry in self.other_scheme_entries
        ):
            raise DomainValidationError(
        ...
        for conflict in self.conflicts:
            if conflict.status is not ConflictStatus.UNRESOLVED:
                raise DomainValidationError(
                    "reconciliation cannot produce resolved conflicts"
                )
